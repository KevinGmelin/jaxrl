...
from jaxrl.wrappers.metaworld_pixel_observation import MetaworldPixelObservationWrapper
...
def make_metaworld_env(ml1,
    ...
    if from_pixels:
        camera_id = 0
        env = MetaworldPixelObservationWrapper(env,
                                      pixels_only=pixels_only,
                                      render_kwargs={'pixels':{'resolution': (image_size, image_size)}})
        env = wrappers.TakeKey(env, take_key='pixels')
        if gray_scale:
            env = wrappers.RGB2Gray(env)
    else:
